...
Restricted sandbox for executing tests generated by the "test" role of the
reflection agent. Tests are Python functions `def check(output, files)
-> dict` that run against cached agent outputs.

The sandbox:
1. Compile the code (syntax check) — ImportError/SyntaxError caught.
2. Executes in restricted namespace with only json/re/os.path/yaml.
3. Detects trivial (always-true) tests that pollute the matrix.
4. Forbid dangerous imports: subprocess, socket, urllib, requests, open(),
   __import__, eval(), exec(), compile(), globals(), locals(), getattr with
   dynamic string.
...
The sandbox does NOT use subprocess to stay lightweight. It runs in the same
process with restricted globals — forbidden imports are detected via
source code inspection before exec(). It is not secure against
dedicated adversaries, but tests are generated by an LLM configured by
...
@dataclass
class TestResult:
    """Result of executing a test in the sandbox."""
    passed: bool
    reason: str = ""
...
class ValidationResult:
    """Result of validating a test before execution."""
    valid: bool
    reason: str = ""
    code_hash: str = ""
...
def _has_required_signature(code: str) -> bool:
    ...
    # Accept: def check(output, files) -> dict:  |  def check(output: str, files: dict) -> dict:
...
def validate_test(code: str) -> ValidationResult:
    """Validates a test before incorporating it into the evidence matrix.
    Returns ValidationResult(valid, reason, code_hash).
    """
...
def _exec_test_sandboxed(code: str, stdout: str, files: dict) -> dict:
    """Executes the test in restricted namespace. Returns the check() dict."""
    ...
    # `import subprocess` or `import socket`.
...
def validate_test_strict(code: str) -> tuple[bool, str]:
    ...
    This catches tests that pass the syntax check but are
...
def run_test_sandboxed(
    ...
    """Executes a validated test against the cached output.
    Returns TestResult with passed, reason, category, error, runtime_s.
    """
    t0 = time.time()
