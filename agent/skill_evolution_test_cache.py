...
Cache of executable tests generated by the reflection agent of the Skill Evolution
Engine (SEE). The tests are expensive Python code to generate (most expensive model,
e.g. Claude Opus); caching them across evolutions of the same skill avoids calling the
LLM again.

Public API: the five TestCache methods replicate the requested signature
(get_cached_test / store_test / invalidate_stale_tests / prune_expired_tests /
archive_refuted_tests). The cache is per-skill, thread-safe and best-effort: any I/O or
locking failure degrades to "miss" without blocking the evolution.
"""
...
@dataclass
class TestCase:
    """An executable test: check(output, files) -> {pass, reason, category}."""

    test_id: str                       # e.g. "T1_check_exit_code"
    ...
    def cache_key(self, skill_hash: str) -> str:
        """Cache key = hash(hypothesis_description + skill_version_hash + observable_behavior)."""
...
class TestCacheError(Exception):
    """Best-effort cache failure; should never abort an evolution."""


class TestCache:
    """Test cache for a skill guarded by advisory lock (fcntl.flock)."""
    ...
    def get_cached_test(self, hypothesis_desc: str, skill_hash: str,
        ...
        """Reuse a valid, non-stale test; increments hit-rate. Best-effort."""
    ...
    def store_test(self, test: TestCase, skill_hash: str) -> None:
        """Persist the test (manifest + T<n>_<slug>.py). Best-effort."""
    ...
    def invalidate_stale_tests(self, current_skill_hash: str) -> list[str]:
        """Mark as stale (do not delete) tests from previous skill versions."""
    ...
    def prune_expired_tests(self, ttl_days: int = DEFAULT_TTL_DAYS) -> int:
        """Remove tests with created_at older than ttl_days. Returns count."""
    ...
    def archive_refuted_tests(self, hypothesis_id: str) -> None:
        """Move to audit archive the tests of a refuted hypothesis."""
        try:
            with self._locked():
                manifest = self._load_manifest()
