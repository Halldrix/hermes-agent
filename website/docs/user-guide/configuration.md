...
dashboard:
  ...
  lease_reclaim_idle_s: 300.0 # Idle floor before a resident dead-lane record stops vouching for its lease (seconds)
...
- `lease_reclaim_idle_s` (default `300`) — how long a resident dead-lane record (transport dead, no turn running, no live delegated work) must sit idle before it stops vouching for its lease and the orphan-lease sweep may reclaim it. Minutes-scale, not TTL-scale: the WS-orphan reaper already ends orderly disconnects in seconds; the sweep only catches lanes that slipped it. Set `0` to reclaim on transport death alone. (`HERMES_TUI_LEASE_RECLAIM_IDLE_S` remains as an internal override.)
