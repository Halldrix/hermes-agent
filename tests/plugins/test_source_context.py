...
def test_verify_quote_rejects_blank_transport_reference():
    frags = (SourceFragment(namespace="weixin", message_id="", reference="", start=0, end=5),)
    _bind("hello", frags)
    with scoped_tool_call():
        ctx = get_tool_source_context()
        assert ctx is not None
...
@pytest.mark.parametrize("rewrite_kind", ["stacked", "bundle"])
def test_gateway_generated_skill_rewrite_invalidates_source_authority(rewrite_kind, monkeypatch):
    """Skill bundles and stacked skills replace transport text, so bind must stay read-only."""
    from gateway.config import Platform
    from gateway.platforms.event import MessageEvent, MessageType
    from gateway.run import GatewayRunner
    from gateway.session import SessionSource
    from plugins.source_context import bind_execution_for_event, source_context_allows_mutation
    from tools.registry import ToolRegistry

    runner = object.__new__(GatewayRunner)
    source = SessionSource(platform=Platform.WEIXIN, chat_id="c1", user_id="u1")
    event = MessageEvent(
        text="/research" if rewrite_kind == "bundle" else "/skill-a /skill-b do XYZ",
        message_type=MessageType.TEXT,
        source=source,
    )
    note_single_source(event, namespace="weixin", message_id="m1", reference="r1")

    if rewrite_kind == "bundle":
        import agent.skill_bundles as bundles

        monkeypatch.setattr(bundles, "resolve_bundle_command_key", lambda command: "/research")
        monkeypatch.setattr(
            bundles,
            "build_bundle_invocation_message",
            lambda *a, **kw: (
                "BUNDLE SCAFFOLD GENERATED BY GATEWAY\n\ngenerated skill body",
                ["skill-a"],
                [],
            ),
        )
        assert runner._hm_bundle_slash_rewrite(event, source, "q1", "research") is True
    else:
        import agent.skill_commands as commands

        monkeypatch.setattr(
            commands, "get_skill_commands", lambda: {"/skill-a": {"name": "skill-a"}}
        )
        monkeypatch.setattr(commands, "resolve_skill_command_key", lambda command: "/skill-a")
        monkeypatch.setattr(
            commands, "split_stacked_skill_commands", lambda instruction: (["/skill-b"], "do XYZ")
        )
        monkeypatch.setattr(
            commands,
            "build_stacked_skill_invocation_message",
            lambda *a, **kw: (
                "STACKED SCAFFOLD GENERATED BY GATEWAY\n\nskill-a\nskill-b",
                ["skill-a", "skill-b"],
                [],
            ),
        )
        assert runner._hm_skill_slash_rewrite(event, source, "q1", "skill-a") is None

    assert event.source_fragments == ()
    token, _ = bind_execution_for_event(
        event=event, session_key="sess-skill-rewrite", run_generation=31
    )
    try:
        seen: dict = {}
        ...
        def _probe(args, **kw):
            ...
            seen["mutation"] = source_context_allows_mutation(
                seen["ctx"], text=event.text
            )
            return "deny"

        registry = ToolRegistry()
        registry.register(
            "probe-generated-skill", "test", {"type": "object", "properties": {}}, _probe
        )
        assert registry.dispatch("probe-generated-skill", {}) == "deny"
        assert seen["ctx"] is not None
        assert seen["mutation"] is False
    finally:
        clear_execution(token)


def test_rebase_failure_after_text_join_drops_stale_authority():
    """Malformed incoming metadata must not survive a presentation-changing merge."""
    ...
    second.source_fragments = (object(),)
    pending: dict = {}
    ...
    assert merged.source_fragments == ()

    token, _ = bind_execution_for_event(
        event=merged, session_key="sess-malformed-rebase", run_generation=32
    )
    try:
        seen: dict = {}
        ...
        def _probe(args, **kw):
            seen["ctx"] = get_tool_source_context()
            seen["mutation"] = source_context_allows_mutation(
                seen["ctx"], text=merged.text
            )
            return "deny"

        registry = ToolRegistry()
        registry.register(
            "probe-malformed-rebase", "test", {"type": "object", "properties": {}}, _probe
        )
        assert registry.dispatch("probe-malformed-rebase", {}) == "deny"
        assert seen["ctx"] is not None
        assert seen["mutation"] is False
    finally:
        clear_execution(token)
...
def test_pre_gateway_dispatch_rewrite_must_invalidate():
    ...
    real_invoke = getattr(lc, "ainvoke_hook", None)
    try:
        async def _fake_ainvoke(name, **kw):
            return [
                {"action": "rewrite", "text": "REWRITTEN PAYLOAD THAT IS LONG ENOUGH FOR SPAN"}
            ] if name == "pre_gateway_dispatch" else []

        lc.ainvoke_hook = _fake_ainvoke
        # main made the hook async (gateway/run_inbound.py::`await ainvoke_hook`) — drive it via asyncio.run.
        import asyncio
        rewritten = asyncio.run(mixin._hm_pre_gateway_dispatch_hook(event, source))
    finally:
        if real_invoke is not None:
            lc.ainvoke_hook = real_invoke
        else:
            try:
                delattr(lc, "ainvoke_hook")
            except Exception:
                pass
